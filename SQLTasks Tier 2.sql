...
Answer -  5: Tennis Court 1, Tennis Court 2, Massage Room 1, Massage Room 2, and Squash Court */
/* SELECT * 
FROM Facilities 
WHERE membercost > 0; */
SELECT
...
SELECT f.name,  
CASE
    WHEN m.surname = 'GUEST' THEN 'GUEST'
...
FROM(
    SELECT f.name as facility,
        CASE 
            WHEN (m.surname = 'GUEST' OR m.firstname = 'GUEST') THEN 'Guest'
            ELSE CONCAT(m.surname,', ', m.firstname)
        END AS membername,
        CASE
            WHEN b.memid = 0 THEN b.slots*f.guestcost
